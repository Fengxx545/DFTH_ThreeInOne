...
public class Ruler extends View implements ValueAnimator.AnimatorListener {
    ...
    private int SelectItemListener;
    ...
    private void init(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.Ruler);
        borderWidth = ta.getDimension(R.styleable.Ruler_rBorderWidth, 8);
        lineWidth = ta.getDimension(R.styleable.Ruler_rLineWidth, 2.0f);
        borderColor = ta.getColor(R.styleable.Ruler_rBorderColor, getResources().getColor(R.color.colorRulerBorder));
        lineColor = ta.getColor(R.styleable.Ruler_rLineColor, getResources().getColor(R.color.colorRulerLine));
        trigonSize = (int) ta.getDimension(R.styleable.Ruler_rTrigonSize, 20);
        pixel = ta.getInt(R.styleable.Ruler_rPixel, 15);
        step = ta.getInt(R.styleable.Ruler_rStep, 1);
        int textSize = (int) ta.getDimension(R.styleable.Ruler_rTextSize, 30);
        int textColor = ta.getColor(R.styleable.Ruler_rTextColor, getResources().getColor(R.color.google_black));
        lineHeight = (int) ta.getDimension(R.styleable.Ruler_rLineHeight, 25);
        lineToText = (int) ta.getDimension(R.styleable.Ruler_rLineToText, 35);
        begin = ta.getInt(R.styleable.Ruler_rBegin, 0);
        SelectItemListener = begin;
        end = ta.getInt(R.styleable.Ruler_rEnd, 1000);
        minVelocity = ta.getInt(R.styleable.Ruler_rMinVelocity, 500);
        animTime = ta.getInt(R.styleable.Ruler_rAnimTime,300);
        indicateHeight = (int) ta.getDimension(R.styleable.Ruler_rIndicateHeight, 0);
        isRect = ta.getBoolean(R.styleable.Ruler_rIsRect, true);
        isTop = ta.getBoolean(R.styleable.Ruler_rIsTop, true);
        ta.recycle();
        ...
        getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                if(null!= onSelectItemListener){
                    SelectItemListener = onSelectItemListener.setSelectItemListener();
                }
                SelectItemListener();
    ...
    private void SelectItemListener() {
        if (SelectItemListener > end || SelectItemListener < begin) {
            throw new RuntimeException("设置所选值超出范围");
        }
        sumMoveX = -(((SelectItemListener-begin) / step * pixel) - halfWidth);
        invalidate();
    }
     ...
     * @param onSelectItemListener 设置选中的Item
     */
    public void setOnSelectItemListener(SelectItemListener onSelectItemListener) {
        this.onSelectItemListener = onSelectItemListener;
    }

    private SelectItemListener onSelectItemListener;

    public interface SelectItemListener{
        int setSelectItemListener();
    }
}
