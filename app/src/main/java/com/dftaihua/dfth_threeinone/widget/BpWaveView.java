...
//    private static final int DEFAULT_WAVE_COLOR = Color.parseColor("#212121");
    ...
    public BpWaveView(Context context) {
        this(context, null);
    }

    public BpWaveView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BpWaveView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.BpWaveView, defStyleAttr, 0);

        // Init ShapeType
        mShapeType = attributes.getInteger(R.styleable.BpWaveView_bwv_shapeType, DEFAULT_WAVE_SHAPE);

        // Init Wave
        mWaveColor = attributes.getColor(R.styleable.BpWaveView_bwv_waveColor, DEFAULT_WAVE_COLOR);
        mWaveBgColor = attributes.getColor(R.styleable.BpWaveView_bwv_wave_background_Color, DEFAULT_WAVE_BACKGROUND_COLOR);

        mWaveBgPaint.setColor(mWaveBgColor);

        // Init AmplitudeRatio
        float amplitudeRatioAttr = attributes.getFloat(R.styleable.BpWaveView_bwv_waveAmplitude, DEFAULT_AMPLITUDE_VALUE) / 1000;
        mAmplitudeRatio = (amplitudeRatioAttr > DEFAULT_AMPLITUDE_RATIO) ? DEFAULT_AMPLITUDE_RATIO : amplitudeRatioAttr;

        // Init Progress
        mProgressValue = attributes.getInt(R.styleable.BpWaveView_bwv_progressValue, DEFAULT_WAVE_PROGRESS_VALUE);
        setProgressValue(mProgressValue);

        // Init RoundRectangle
        mIsRoundRectangle = attributes.getBoolean(R.styleable.BpWaveView_bwv_round_rectangle, false);
        mRoundRectangleXY = attributes.getInteger(R.styleable.BpWaveView_bwv_round_rectangle_x_and_y, DEFAULT_ROUND_RECTANGLE_X_AND_Y);

        // Init Triangle direction
        mTriangleDirection = attributes.getInteger(R.styleable.BpWaveView_bwv_triangle_direction, DEFAULT_TRIANGLE_DIRECTION);

        // Init Border
        mBorderPaint = new Paint();
        mBorderPaint.setAntiAlias(true);
        mBorderPaint.setStyle(Paint.Style.STROKE);
        mBorderPaint.setStrokeWidth(attributes.getDimension(R.styleable.BpWaveView_bwv_borderWidth, dp2px(DEFAULT_BORDER_WIDTH)));
        mBorderPaint.setColor(attributes.getColor(R.styleable.BpWaveView_bwv_borderColor, DEFAULT_WAVE_COLOR));

        // Init Top Title
        mTopTitlePaint = new Paint();
        mTopTitlePaint.setColor(attributes.getColor(R.styleable.BpWaveView_bwv_titleTopColor, DEFAULT_TITLE_COLOR));
        mTopTitlePaint.setStyle(Paint.Style.FILL);
        mTopTitlePaint.setAntiAlias(true);
        mTopTitlePaint.setTextSize(attributes.getDimension(R.styleable.BpWaveView_bwv_titleTopSize, sp2px(DEFAULT_TITLE_TOP_SIZE)));

        mTopTitleStrokePaint = new Paint();
        mTopTitleStrokePaint.setColor(attributes.getColor(R.styleable.BpWaveView_bwv_titleTopStrokeColor, DEFAULT_STROKE_COLOR));
        mTopTitleStrokePaint.setStrokeWidth(attributes.getDimension(R.styleable.BpWaveView_bwv_titleTopStrokeWidth, dp2px(DEFAULT_TITLE_STROKE_WIDTH)));
        ...
        mTopTitle = attributes.getString(R.styleable.BpWaveView_bwv_titleTop);

        // Init Center Title
        mCenterTitlePaint = new Paint();
        mCenterTitlePaint.setColor(attributes.getColor(R.styleable.BpWaveView_bwv_titleCenterColor, DEFAULT_TITLE_COLOR));
        mCenterTitlePaint.setStyle(Paint.Style.FILL);
        mCenterTitlePaint.setAntiAlias(true);
        mCenterTitlePaint.setTextSize(attributes.getDimension(R.styleable.BpWaveView_bwv_titleCenterSize, sp2px(DEFAULT_TITLE_CENTER_SIZE)));

        mCenterTitleStrokePaint = new Paint();
        mCenterTitleStrokePaint.setColor(attributes.getColor(R.styleable.BpWaveView_bwv_titleCenterStrokeColor, DEFAULT_STROKE_COLOR));
        mCenterTitleStrokePaint.setStrokeWidth(attributes.getDimension(R.styleable.BpWaveView_bwv_titleCenterStrokeWidth, dp2px(DEFAULT_TITLE_STROKE_WIDTH)));
        ...
        mCenterTitle = attributes.getString(R.styleable.BpWaveView_bwv_titleCenter);

        // Init Bottom Title
        mBottomTitlePaint = new Paint();
        mBottomTitlePaint.setColor(attributes.getColor(R.styleable.BpWaveView_bwv_titleBottomColor, DEFAULT_TITLE_COLOR));
        mBottomTitlePaint.setStyle(Paint.Style.FILL);
        mBottomTitlePaint.setAntiAlias(true);
        mBottomTitlePaint.setTextSize(attributes.getDimension(R.styleable.BpWaveView_bwv_titleBottomSize, sp2px(DEFAULT_TITLE_BOTTOM_SIZE)));

        mBottomTitleStrokePaint = new Paint();
        mBottomTitleStrokePaint.setColor(attributes.getColor(R.styleable.BpWaveView_bwv_titleBottomStrokeColor, DEFAULT_STROKE_COLOR));
        mBottomTitleStrokePaint.setStrokeWidth(attributes.getDimension(R.styleable.BpWaveView_bwv_titleBottomStrokeWidth, dp2px(DEFAULT_TITLE_STROKE_WIDTH)));
        ...
        mBottomTitle = attributes.getString(R.styleable.BpWaveView_bwv_titleBottom);
...
//            paint.setAlpha(50);
        ...
        for (int i = 0; i < 6; i++) {
//            String text = String.valueOf(60 * (5 - i));
            String text = BpDataUtils.getDefaultUnitValue(getContext(), 60 * (5 - i));
            int textWidth = TextViewUtils.getStringWidth(text, sp2px(20));
            float y = this.getHeight() / 6 * i + this.getHeight() / 12;
            canvas.drawLine(0, y, 50, y, paint);
            canvas.drawText(text, 50 + textWidth / 2 + 10, y + 20, paint);
            float x = this.getWidth();
            canvas.drawLine(x, y, x - 50, y, paint);
            canvas.drawText(text, x - (50 + textWidth / 2 + 10), y + 20, paint);
        }
    }

    private void drawBack(Canvas canvas) {
        Paint p = new Paint();
        p.setColor(ThreeInOneApplication.getColorRes(R.color.bp_measure_bottom));
        canvas.drawPaint(p);
    }
...
//                wavePaint.setColor(adjustAlpha(mWaveColor, 0.3f));
                wavePaint.setColor(ThreeInOneApplication.getColorRes(R.color.bp_measure_wave_minor));
...
//        mWaveShader = null;
    ...
    public void setProgressValue(int progress) {
        mProgressValue = progress + 30;
        ObjectAnimator waterLevelAnim = ObjectAnimator.ofFloat(this, "waterLevelRatio", mWaterLevelRatio, ((float) mProgressValue / 360));
